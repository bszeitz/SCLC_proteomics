...
CM.expr.imp.secr.sd.scaled <- t(scale(t(CM.expr.imp.secr.sd)))
...
PVCA.CP <- plot_PVCA(as.data.frame(CP.expr.imp.sd), Annot, "Sample", 
          ...
          "", F)
PVCA.CP$Data <- "CP"
PVCA.CM <- plot_PVCA(as.data.frame(CM.expr.imp.secr.sd), Annot, "Sample", 
          annot.columns, pct.threshold = 0.6, color="blue", 
          "", F)
PVCA.CM$Data <- "CM"
...
CP.PCA
CM.secr.PCA <- ggbiplot(prcomp(t(na.omit(CM.expr.imp.secr.sd.scaled))), 
                        choices= c(1,2),
                        var.axes=FALSE, 
                        #labels=Annot$Name,
                        groups=Annot$Subtype,
  ...
  annotate("text", x=-0.3, y=-2, label= "Adh",fontface =2,  color="black")+ggtitle("CM secreted proteins (SD > 1.25)")+ 
  geom_point(aes(shape=culture.type, color=Annot$Subtype), size=3)+scale_shape_manual(values=c(16, 8, 15))+
  scale_color_manual(values=c("#0000FE","#21FE06","gold","#FB02FE"))+ xlab("PC1 (20.1% explained var.)")+
  ylab("PC2 (12.0% explained var.)")

CM.secr.PCA
```

```{r , fig.width=12, fig.height=5}
ggarrange(CP.PCA, CM.secr.PCA, 
          ncol=2, nrow=1)
...
```{r }
pdf(file="Figures/Consclust_heatmap.pdf",width = 12, height = 12)
ht = draw(ht, annotation_legend_side="bottom",
             heatmap_legend_side="left")
dev.off()
