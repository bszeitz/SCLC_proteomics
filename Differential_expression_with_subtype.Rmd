...
CM.Subtype.ANOVA.SPs <- CM.Subtype.ANOVA[secreted.proteins.CM,]
...
Subtype.specific <- lapply(names(Subtype.specific), function(st){
  ...
  CM.sign <- CM.Subtype.ANOVA.SPs[CM.Subtype.ANOVA.SPs$`p.adj Subtype` < 0.05,]
  CM.sign$Data <- "CM"
...
save(ORA.SubtypeSpecific, file="RData/ORA_SubtypeSpecific.RData")
...
```{r }
load("RData/ORA_SubtypeSpecific.RData")
...
Subtype.specific.common <- rbind(Subtype.specific[[1]], Subtype.specific[[2]], Subtype.specific[[3]], Subtype.specific[[4]])
...
saveWorkbook(wb, paste0("Files/Supplementary_SubtypeSpecific_ORA.xlsx"), overwrite = TRUE)
...
```{r }
save(ORA.SubtypeSpecific, Protein.to.gene, file= "RData/Fig_CNET_SubtypeSpecific.RData")
```
